...
export default function Summary() {
  ...
  if (!a) return <div className="card">Resumo não encontrado.</div>;
  const doc = doctorById(a.doctorId);

  return (
    <>
      <button className="btn-ghost btn-sm" style={{ marginBottom: 8 }} onClick={() => navigate('/patient/history')}>
        <Icon name="arrowLeft" /> Voltar
      </button>
      <h1 className="page-title" style={{ fontSize: 24 }}>Resumo da consulta</h1>
      <div className="page-sub">{doc.name} · {apptDateLabel(a)}</div>
