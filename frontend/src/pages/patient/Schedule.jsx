  ...
  return (
    <>
      ...
      <div className="page-sub">
        Escolha a área médica para ver os profissionais disponíveis.
      </div>
      <input
        className="input"
    ...
    <>
      ...
      <div className="page-sub">
        ...
        <b style={{ color: 'var(--texto)' }}>
          {specialtyById(booking.specialty)?.name}
        </b>
      </div>
      <div className="grid-3" style={{ gridTemplateColumns: 'repeat(5, 1fr)' }}>
        {days.map((d) => {
          const key = dateKey(d);
          const hasAvail = DOCTORS.some(
            (doc) => doc.specialty === booking.specialty && doc.slots[key],
          );
          return (
            <button
              key={key}
            ...
            >
              ...
              <div className="num" style={{ fontSize: 20, fontWeight: 500, margin: '4px 0' }}>
                {d.getDate()}
              </div>
              <div className="small muted">
              ...
              {hasAvail && (
                <div
                  style={{
                    ...
                    background: 'var(--celeste-500)', margin: '6px auto 0',
                  }}
                />
              )}
            </button>
...
export function ScheduleDoctor() {
  ...
  let docs = DOCTORS.filter(
    (d) => d.specialty === booking.specialty && d.slots[booking.date],
  );
  if (booking.sort === 'rating') docs = [...docs].sort((a, z) => z.rating - a.rating);
  if (booking.sort === 'slots')
    docs = [...docs].sort(
      (a, z) => z.slots[booking.date].length - a.slots[booking.date].length,
    );
  if (booking.sort === 'name')
    docs = [...docs].sort((a, z) => a.name.localeCompare(z.name));
              ...
              <div>
                ...
                <div style={{ marginTop: 6 }}>
                  <span className="badge badge-success">
                    <Icon name="check" size={12} /> CRM verificado
                  </span>
                </div>
              </div>
            </div>
            <div className="small muted" style={{ margin: '12px 0' }}>{doc.bio}</div>
            <div className="small" style={{ color: 'var(--texto-2)', marginBottom: 8 }}>
...
export function ScheduleConfirm() {
  ...
  const onConfirm = () => {
    setBooking((b) => ({
      ...b,
      reason: motivo.trim() || 'Consulta agendada pelo paciente',
    }));
...
export function ScheduleCode() {
  const navigate = useNavigate();
  const { booking, addAppointment, resetBooking, pushNotification, doctorById } =
    useApp();
  ...
  const onFinish = () => {
    ...
    pushNotification(
      `Consulta com ${doctorById(newAppt.doctorId).name} confirmada para ${d.toLocaleDateString(
